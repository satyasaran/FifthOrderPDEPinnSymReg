...
def create_model(config):
    ...
    #(3.0/5)*np.pi * x[:, 0:1]
    #ic = dde.IC(geomtime, lambda x: np.exp(-2*(x[:, 0:1]-3)**2)+0.1*np.sin((3.0/5)*np.pi * x[:, 0:1]), lambda _, on_initial: on_initial)
    
    ic = dde.icbc.IC(geomtime, lambda x:  np.cos(3*np.pi * x[:, 0:1])+np.exp(-20*x[:, 0:1]**2), lambda _, on_initial: on_initial
)
...
def train_model(model, config):
    ...
    losshistory, train_state =model.train(iterations=20000,display_every=2000,model_save_path='Nomagnetic_5th_6'+str(ITERATION)+'/model',callbacks=[early_stopping])
...
dump(search_result, 'results_comb_gp4_a100_exp_COS.pkl', store_objective=False)
