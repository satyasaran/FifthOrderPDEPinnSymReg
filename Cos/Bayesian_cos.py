...
def create_model(config):
    ...
    ic = dde.icbc.IC(geomtime, lambda x:  np.cos(3*np.pi * x[:, 0:1])+0*np.exp(-20*x[:, 0:1]**2), lambda _, on_initial: on_initial
)
...
dump(search_result, 'results_comb_gp4_a100_COS.pkl', store_objective=False)
